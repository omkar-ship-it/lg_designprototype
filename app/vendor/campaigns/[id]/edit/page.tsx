        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}>
          <Section title="Participation Settings">
            <div className="space-y-5">
              ...
              {/* Plays Per Day — not for stamp, lottery, buy x get y, or coupon */}
              {!isStamp && !isLottery && !isBuyXGetY && !isCoupon && (
                isLocked ? (
              ...
              {/* Daily Reward Cap — not for stamp, lottery, buy x get y, or coupon */}
              {!isStamp && !isLottery && !isBuyXGetY && !isCoupon && (
                isLocked ? (
              ...
              {/* Stamp / Lottery / Buy X Get Y / Coupon notes */}
              {(isStamp || isLottery || isBuyXGetY || isCoupon) && !isLocked && (
                <div className="flex items-start gap-2 p-3 bg-v-surface-2 border border-v-border rounded-xl text-xs text-v-text-2">
                  {isStamp ? 'Stamp Card has no plays per day or daily reward cap — rewards trigger at stamp positions.' :
                   ...
                   'Coupon Codes has no plays per day or daily reward cap — the coupon pool size governs total redemptions.'}
                </div>
              )}
            </div>
          </Section>
