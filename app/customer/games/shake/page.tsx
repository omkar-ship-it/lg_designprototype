  ...
  return (
    ...
    >
      ...
      {state === 'idle' && SPARKLE_POS.map((pos, i) => (
        <motion.div
          key={i}
          className="absolute text-blue-300/30 text-xs pointer-events-none select-none"
          style={pos}
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div>
            <h2 className="text-xl font-bold text-white">☁️ Shake & Win</h2>
            <p className="text-xs text-white/60 mt-1">Shake your phone to reveal rewards</p>
          </div>
          <span className="px-3 py-1.5 rounded-full text-xs font-semibold" style={{ background: '#10B981', color: '#fff' }}>LIVE</span>
        </div>
        <div className="grid grid-cols-3 gap-4 text-xs">
          <div>
            <p className="text-white/40 mb-1 uppercase tracking-wide text-[10px]">Win Chance</p>
            <p className="text-white font-bold text-sm">65%</p>
          </div>
          <div>
            <p className="text-white/40 mb-1 uppercase tracking-wide text-[10px]">Duration</p>
            <p className="text-white font-bold text-sm">Jun 13–Jul 15</p>
          </div>
          <div>
            <p className="text-white/40 mb-1 uppercase tracking-wide text-[10px]">Plays Left</p>
            <p className="text-white font-bold text-sm">{playsLeft}</p>
          </div>
        </div>
      </motion.div>

      {/* Back */}
      <button onClick={() => router.back()} className="flex items-center gap-1.5 text-white/50 text-sm hover:text-white/70 transition-colors mb-6 self-start relative z-10 px-5">
      ...
      <motion.div
        onClick={scratch}
        className="relative z-10 rounded-3xl overflow-hidden select-none"
        style={{
          width: '288px',
      ...
      >
        ...
        <div style={{ background: 'linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%)', padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="flex items-center justify-between relative">
            ...
            <span className="text-[10px] font-bold text-white/40 bg-white/10 px-2 py-0.5 rounded-full">65% WIN</span>
          </div>
        </div>

        {/* Scratch area */}
        <div className="relative bg-white overflow-hidden" style={{ height: '210px' }}>
            ...
            <AnimatePresence>
              {state !== 'idle' && (
                ...
                >
                  ...
                  <p className="text-base font-extrabold text-gray-800">{won ? REWARD : 'Not this time!'}</p>
      ...
      >
        Each visit earns you one scratch · Results are instant
      </motion.p>
    </div>
  )
}
