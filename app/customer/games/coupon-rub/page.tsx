...
const REWARD_LABEL = '15% Off'
const COUPON_CODE = 'AMBER15'
...
export default function CouponRubPage() {
        ...
        <div className="text-center mb-6 min-h-[60px] flex flex-col items-center justify-center">
          <AnimatePresence mode="wait">
            {claimed ? (
              ...
              >
                <p className="text-gray-900 text-[22px] font-bold leading-snug">🎟️ Your code is unlocked!</p>
                <p className="text-gray-500 text-sm mt-1.5">{BUSINESS_NAME} {BUSINESS_EMOJI}</p>
              </motion.div>
            ) : (
              <motion.div key="claim-title" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <p className="text-gray-500 text-sm mt-1.5">Reveal your surprise discount code</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <AnimatePresence>
          {!claimed && (
            ...
            >
              ...
              >
                {claiming ? '✨ Revealing your code…' : '🪔 Rub to Reveal'}
              </motion.button>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {showReward && (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-gray-900 font-bold text-[15px] leading-tight">{REWARD_LABEL} at {BUSINESS_NAME}</p>
                  <p className="text-[11px] mt-0.5 text-gray-400 font-mono tracking-wider">CODE: {COUPON_CODE}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 text-[10px] flex-wrap text-gray-400">
                ...
                <span className="font-semibold text-gray-700">{fmtDate(CLAIM_BEFORE)}</span>
                <span className="opacity-30 mx-0.5">|</span>
                <div className="flex items-center gap-1">
                  <Gift className="w-3 h-3" />
                  <span>Redeem Before</span>
                </div>
                <span className="font-semibold text-gray-700">{fmtDate(REDEEM_BEFORE)}</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
